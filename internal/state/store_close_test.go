...
func TestStoreCloseReleasesPinnedDirPathOnLinuxProcFS(t *testing.T) {
	...
	if _, err := os.Stat(pinnedRoot); err != nil {
		t.Fatalf("pinned root unavailable before Close: %v", err)
	}
	if err := st.Close(); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(pinnedRoot); err == nil {
		t.Fatalf("pinned root %q remained resolvable after Close", pinnedRoot)
	}
}
