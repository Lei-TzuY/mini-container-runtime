...
func buildSecurityDockerfile(t *testing.T, contextDir, outputDir, content string) error {
	...
	_, err := BuildDockerfile(BuildOptions{
		...
		Dockerfile: writeDockerfileForSecurityTest(t, contextDir, content),
		Tag:        "security:test",
		OutputDir:  outputDir,
	})
	return err
...
func TestBuildRejectsCopySourceTraversal(t *testing.T) {
	...
	if _, statErr := os.Stat(filepath.Join(outputDir, "leak.txt")); !os.IsNotExist(statErr) {
		t.Fatalf("traversal copied host secret into output: err=%v", statErr)
	}
}
...
func TestBuildRejectsCopySourceSymlinkOutsideContext(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "symlink") {
		t.Fatalf("COPY symlink error=%v", err)
	}
}
...
func TestBuildRejectsSymlinkInsideCopiedContextTree(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "contains symlink") {
		t.Fatalf("COPY tree symlink error=%v", err)
	}
}
...
func TestBuildParentTraversalStaysInsideOutputRoot(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("contained traversal build failed: %v", err)
	}
	...
	if string(data) != "sentinel" {
		t.Fatalf("host sibling was overwritten: %q", data)
	}
	inside, err := os.ReadFile(filepath.Join(outputDir, "pwned.txt"))
	if err != nil {
		t.Fatalf("contained RUN output missing: %v", err)
	}
	if string(inside) != "safe\n" {
		t.Fatalf("contained RUN output=%q", inside)
	}
}
...
func TestBuildTreatsAbsoluteRootFSSymlinkAsContainerRelative(t *testing.T) {
	...
	outsideFile := filepath.Join(outsideDir, "written.txt")
	if err := os.WriteFile(outsideFile, []byte("sentinel"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink(outsideDir, filepath.Join(outputDir, "escape")); err != nil {
		t.Fatal(err)
	}

	err := buildSecurityDockerfile(t, contextDir, outputDir, "FROM scratch\nRUN echo contained > /escape/written.txt\n")
	if err != nil {
		t.Fatalf("build through rootfs symlink: %v", err)
	}
	...
	if string(outsideData) != "sentinel" {
		t.Fatalf("host symlink target was overwritten: %q", outsideData)
	}

	logicalTarget := filepath.Join(outputDir, strings.TrimPrefix(filepath.ToSlash(outsideDir), "/"), "written.txt")
	insideData, err := os.ReadFile(logicalTarget)
	if err != nil {
		t.Fatalf("container-relative symlink target missing: %v", err)
	}
	if string(insideData) != "contained\n" {
		t.Fatalf("container-relative target=%q", insideData)
	}
}
