...
func TestEntriesWithRegistrationRejectsLiveForeignContainerOwner(t *testing.T) {
	...
	got, changed, err := entriesWithRegistration(entries, registrarIdentity{PID: 303, StartTime: 3003}, "ctr-a", "app-a", "172.20.0.2")
	if err == nil || !strings.Contains(err.Error(), "live DNS registration conflict") {
		t.Fatalf("foreign container owner was accepted: got=%+v changed=%v err=%v", got, changed, err)
	}
	if changed || got != nil {
		t.Fatalf("foreign conflict exposed replacement state: got=%+v changed=%v", got, changed)
	}
	if !reflect.DeepEqual(entries, []HostEntry{foreign}) {
		t.Fatalf("foreign conflict mutated input: %+v", entries)
	}
}

func TestEntriesWithRegistrationRejectsLiveForeignHostnameOwner(t *testing.T) {
	entries := []HostEntry{{
		ContainerID:    "ctr-newer",
		Hostname:       "shared-name",
		IP:             "172.20.0.9",
		OwnerPID:       404,
		OwnerStartTime: 4004,
	}}
	_, changed, err := entriesWithRegistration(entries, registrarIdentity{PID: 505, StartTime: 5005}, "ctr-stale", "shared-name", "172.20.0.2")
	if err == nil {
		t.Fatal("foreign hostname owner was accepted")
	}
	if changed {
		t.Fatal("foreign hostname conflict reported mutation")
	}
}
...
func TestEntriesWithRegistrationAppendsDistinctRegistration(t *testing.T) {
	...
	got, changed, err := entriesWithRegistration(entries, owner, "ctr-b", "app-b", "172.20.0.2")
	if err != nil {
		t.Fatalf("distinct registration: %v", err)
	}
