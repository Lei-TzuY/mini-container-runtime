...
func CommitPendingExec() error {
	mu.Lock()
	defer mu.Unlock()
	if len(stagedExecs) == 0 { return nil }
	if len(stagedExecs) != 1 { return fmt.Errorf("cannot commit exec event: %d staged exec events", len(stagedExecs)) }
	if len(activeExecs) != 0 { return fmt.Errorf("cannot commit exec event: %d active exec events", len(activeExecs)) }
	var staged stagedExecEvent
	for _, candidate := range stagedExecs { staged = candidate }
	delete(stagedExecs, staged.containerID)
	activeExecs[staged.containerID] = staged
	return appendEventUnlocked(Event{Timestamp: time.Now(), Type: EventExec, ContainerID: staged.containerID, Image: staged.image, Message: staged.message})
}

func CompletePendingExec(exitCode int, detail string) error {
	mu.Lock()
	defer mu.Unlock()
	if len(activeExecs) == 0 { return nil }
	if len(activeExecs) != 1 { return fmt.Errorf("cannot complete exec event: %d active exec events", len(activeExecs)) }
	var active stagedExecEvent
	for _, candidate := range activeExecs { active = candidate }
...
func FailPendingExec(detail string) error {
	...
	defer mu.Unlock()
	if len(stagedExecs) == 0 { return nil }
	if len(stagedExecs) != 1 { return fmt.Errorf("cannot fail exec event: %d staged exec events", len(stagedExecs)) }
	var staged stagedExecEvent
	for _, candidate := range stagedExecs { staged = candidate }
	delete(stagedExecs, staged.containerID)
...
func DiscardPendingExec() error {
	...
	defer mu.Unlock()
	if len(stagedExecs) == 0 { return nil }
	if len(stagedExecs) != 1 { return fmt.Errorf("cannot discard exec event: %d staged exec events", len(stagedExecs)) }
	for containerID := range stagedExecs { delete(stagedExecs, containerID) }
	return nil
}
