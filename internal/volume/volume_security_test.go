...
func TestVolumeRootSymlinkCannotCreateOrDeleteOutside(t *testing.T) {
	...
	if _, err := CreateVolume("newvol"); err == nil || !strings.Contains(err.Error(), "volume storage directory is not a real directory") {
		t.Fatalf("CreateVolume symlink root error=%v", err)
	}
	if err := RemoveVolume("victim"); err == nil || !strings.Contains(err.Error(), "volume storage directory is not a real directory") {
		t.Fatalf("RemoveVolume symlink root error=%v", err)
	}
	if data, err := os.ReadFile(sentinel); err != nil || string(data) != "keep" {
		t.Fatalf("outside victim changed: data=%q err=%v", data, err)
	}
	if _, err := os.Stat(filepath.Join(outside, "newvol")); !os.IsNotExist(err) {
		t.Fatalf("CreateVolume wrote through symlinked root: %v", err)
	}
}
...
func TestVolumeStateBaseSymlinkRejected(t *testing.T) {
	...
	if _, err := CreateVolume("db"); err == nil || !strings.Contains(err.Error(), "state directory is not a real directory") {
		t.Fatalf("CreateVolume symlink state base error=%v", err)
	}
	if _, err := os.Stat(filepath.Join(outside, "volumes", "db")); !os.IsNotExist(err) {
		t.Fatalf("symlinked state base received volume data: %v", err)
	}
}
...
func TestCreateVolumeRejectsSymlinkedVolumeDirectory(t *testing.T) {
	...
	if _, err := CreateVolume("db"); err == nil || !strings.Contains(err.Error(), "not a real directory") {
		t.Fatalf("CreateVolume symlink dir error=%v", err)
	}
	if _, err := os.Stat(filepath.Join(outside, "volume.json")); !os.IsNotExist(err) {
		t.Fatalf("symlinked volume dir received metadata: %v", err)
	}
}
...
func TestCreateVolumeRejectsSymlinkedDataDirectory(t *testing.T) {
	...
	root, err := volumeRoot(true)
	if err != nil { t.Fatal(err) }
	volDir := filepath.Join(root, "db")
	if err := os.Mkdir(volDir, 0o700); err != nil { t.Fatal(err) }
	outside := t.TempDir()
	if err := os.Symlink(outside, filepath.Join(volDir, "_data")); err != nil { t.Fatal(err) }
	if _, err := CreateVolume("db"); err == nil || !strings.Contains(err.Error(), "data directory") {
		t.Fatalf("CreateVolume symlink data error=%v", err)
	}
	if _, err := os.Stat(filepath.Join(volDir, "volume.json")); !os.IsNotExist(err) {
		t.Fatalf("metadata written despite symlink data dir: %v", err)
	}
}
...
func TestCreateVolumeRejectsSymlinkedMetadata(t *testing.T) {
	...
	root, err := volumeRoot(true)
	if err != nil { t.Fatal(err) }
	volDir, _, err := ensureVolumeLayout(root, "db", true)
	if err != nil { t.Fatal(err) }
	...
	if _, err := CreateVolume("db"); err == nil || !strings.Contains(err.Error(), "metadata is not a regular file") {
		t.Fatalf("CreateVolume symlink metadata error=%v", err)
	}
	if data, err := os.ReadFile(outside); err != nil || string(data) != sentinel {
		t.Fatalf("metadata symlink target changed: data=%q err=%v", data, err)
	}
}
...
func TestGetAndRemoveVolumeRejectTamperedMountPath(t *testing.T) {
	...
	if _, err := GetVolume("db"); err == nil || !strings.Contains(err.Error(), "does not match managed data path") {
		t.Fatalf("GetVolume tampered mount path error=%v", err)
	}
	if got := ResolveVolumePath("db"); got != "db" {
		t.Fatalf("ResolveVolumePath trusted tampered mount path: %q", got)
	}
	if err := RemoveVolume("db"); err == nil || !strings.Contains(err.Error(), "validate volume before removal") {
		t.Fatalf("RemoveVolume tampered metadata error=%v", err)
	}
	if data, err := os.ReadFile(sentinel); err != nil || string(data) != "keep" {
		t.Fatalf("volume data changed after rejected removal: data=%q err=%v", data, err)
	}
}

func TestVolumeStoragePrivateMetadataAndUpdate(t *testing.T) {
	setVolumeTestHome(t)
	vol, err := CreateVolume("db0")
	if err != nil { t.Fatal(err) }
	keep := filepath.Join(vol.MountPath, "keep")
	if err := os.WriteFile(keep, []byte("data"), 0o600); err != nil { t.Fatal(err) }
	if _, err := CreateVolume("db0"); err != nil { t.Fatalf("recreate volume: %v", err) }
	if data, err := os.ReadFile(keep); err != nil || string(data) != "data" {
		t.Fatalf("recreate destroyed volume data: data=%q err=%v", data, err)
	}
