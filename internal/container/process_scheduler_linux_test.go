...
const processSchedulerProbeEnv = "MINICONTAINER_TEST_PROCESS_SCHEDULER_PROBE"
...
func TestProcessSchedulerRuntimeMarkerApplied(t *testing.T) {
	...
	cmd := exec.Command(exe, "-test.run=^TestProcessSchedulerRuntimeMarkerApplied$")
	env := make([]string, 0, len(os.Environ())+3)
	for _, entry := range os.Environ() {
		if strings.HasPrefix(entry, sentinelEnvKey+"=") ||
			strings.HasPrefix(entry, processSchedulerEnv+"=") ||
			strings.HasPrefix(entry, processSchedulerProbeEnv+"=") {
			continue
		}
	...
	env = append(env,
		sentinelEnvKey+"=1",
		processSchedulerEnv+"=SCHED_BATCH:5",
		processSchedulerProbeEnv+"=1")
	cmd.Env = env
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("run scheduler probe: %v: %s", err, out)
	}
