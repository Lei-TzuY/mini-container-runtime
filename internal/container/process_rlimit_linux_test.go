...
func TestProcessRlimitRuntimeMarkersApplied(t *testing.T) {
	if os.Getenv(processRlimitProbeEnv) == "1" {
		var nofile, core, fsize, stack unix.Rlimit
		...
		if err := unix.Getrlimit(unix.RLIMIT_STACK, &stack); err != nil {
			fmt.Fprintf(os.Stderr, "getrlimit STACK: %v", err)
			os.Exit(2)
		}
		fmt.Printf("%d:%d|%d:%d|%d:%d|%d:%d|%q|%q|%q|%q",
			...
			os.Getenv(processRlimitSTACKEnv),
		)
		os.Exit(0)
	}
	...
	if err := unix.Getrlimit(unix.RLIMIT_STACK, &currentStack); err != nil {
		t.Fatalf("get current RLIMIT_STACK: %v", err)
	}
	const minimumStack = 1 << 20
	const preferredStack = 8 << 20
	if currentStack.Max < minimumStack {
		t.Skipf("RLIMIT_STACK hard limit %d is too small for deterministic probe", currentStack.Max)
	}
	stackLimit := currentStack.Max
	if stackLimit > preferredStack {
		stackLimit = preferredStack
	}
	...
	for _, entry := range os.Environ() {
		if strings.HasPrefix(entry, sentinelEnvKey+"=") ||
			...
			strings.HasPrefix(entry, processRlimitSTACKEnv+"=") ||
	...
	want := fmt.Sprintf("%d:%d|0:0|4096:8192|%d:%d|%q|%q|%q|%q", soft, hard, stackLimit, stackLimit, "", "", "", "")
