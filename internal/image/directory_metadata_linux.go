...
func finalizeDirectoryMetadata(destDir string, dirs []directoryMetadata) error {
	...
	for _, meta := range ordered {
		...
		if !meta.modTime.IsZero() {
			...
			if err := unix.Futimes(fd, []unix.Timeval{tv, tv}); err != nil {
				_ = unix.Close(fd)
				return fmt.Errorf("restore directory mtime %s: %w", meta.target, err)
			}
		}
		if err := unix.Close(fd); err != nil {
			return fmt.Errorf("close directory metadata fd %s: %w", meta.target, err)
		}
	}
	return nil
}
