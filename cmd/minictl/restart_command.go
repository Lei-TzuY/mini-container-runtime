...
func defaultRestartCommandDeps() restartCommandDeps {
	return restartCommandDeps{openStore: openStore, stat: os.Stat, run: container.RunWithSecurityPolicy}
}
...
func restartStoppedContainer(idOrPrefix string, deps restartCommandDeps) (*state.Container, error) {
	...
	for _, p := range spec.PortMappings {
		portMappings = append(portMappings, container.PortMapping{HostPort: p.HostPort, ContainerPort: p.ContainerPort, Protocol: p.Protocol})
	}
	volumes := make([]container.Volume, 0, len(spec.Volumes))
	for _, v := range spec.Volumes {
		volumes = append(volumes, container.Volume{HostPath: v.HostPath, ContainerPath: v.ContainerPath, ReadOnly: v.ReadOnly})
	}
	cfg := container.Config{
		ContainerID: rec.ID, StateDir: st.Dir(), RootFS: spec.RootFS, RootFSIdentity: rootfsIdentity,
		Overlay: spec.Overlay, ReadOnly: spec.ReadOnly, Restart: spec.Restart, CapDrop: append([]string(nil), spec.CapDrop...), NoNewPrivileges: spec.NoNewPrivileges,
		Command: append([]string(nil), spec.Command...), Hostname: spec.Hostname, WorkDir: spec.WorkDir, Env: append([]string(nil), spec.Env...),
		Memory: spec.Memory, CPUWeight: spec.CPUWeight, CPUs: spec.CPUs, PidsLimit: spec.PidsLimit, Seccomp: spec.Seccomp,
		BridgeNetwork: spec.BridgeNetwork, PortMappings: portMappings, Volumes: volumes, UserNS: spec.UserNS, CgroupNS: spec.CgroupNS, Debug: spec.Debug,
	}
