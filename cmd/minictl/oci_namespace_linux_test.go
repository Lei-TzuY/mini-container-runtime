...
func TestOCIBundleRunPreservesCgroupNamespaceSelection(t *testing.T) {
	stateDir := t.TempDir()
	bundle := writeOCIBundle(t, `{"ociVersion":"1.1.0","root":{"path":"rootfs"},"process":{"args":["/bin/sh","-c","printf cgroup-namespace-policy"],"cwd":"/"},"linux":{"namespaces":[{"type":"cgroup"}]}}`)
	...
	id, err := runOCIBundleWith(bundle, ociBundleRunDeps{
		...
		prepare: func(cfg *container.Config) (*state.Store, *state.Container, error) {
			return prepareManagedRunStateWith(cfg, runAdmissionDeps{
				openStore: func() (*state.Store, error) { return state.Open(stateDir) },
				newID:     func() (string, error) { return "oci-cgroupns", nil },
				now:       func() time.Time { return time.Unix(30, 0) },
			})
		},
		run: func(cfg container.Config) error {
			if !cfg.CgroupNS {
				t.Fatal("runner lost OCI cgroup namespace selection")
			}
			out, err := exec.Command("/bin/sh", "-c", "printf cgroup-namespace-policy").Output()
			if err != nil {
				return err
			}
			if string(out) != "cgroup-namespace-policy" {
				t.Fatalf("process output = %q", out)
			}
			return nil
		},
	...
	if !spec.CgroupNS {
		t.Fatalf("restart spec lost cgroup namespace policy: %+v", spec)
	}
}
